...
*/
    ...
    {   
        ...
        for(uint16_t k = 0; k < DURATION_IN_SEC; k++){
            for(uint8_t i = 0; i < imu_sampling_rate; i++){
                acc_x = data_imu[i + k*(imu_sampling_rate)][1];
                ...
                omega = data_imu[i + k*(imu_sampling_rate)][0] * (180.0/PI);
                ...
                *EEKF_MAT_EL(u, 1, 0) = omega;
                ...
                if(SysTick->VAL > prediction_systick_timer)
                    prediction_systick_timer = SysTick->VAL - prediction_systick_timer;
                else 
            ...
            *EEKF_MAT_EL(z, 1, 0) = data_gps[k][1];
            ...
            if(SysTick->VAL > correction_systick_timer)
                correction_systick_timer = SysTick->VAL - correction_systick_timer;
            else 
